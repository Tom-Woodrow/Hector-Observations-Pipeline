...
def correct_parking_positions_file(
    ...
    df = apply_corrections(
        ...
        apply_telecentricity_correction=False,
...
def correct_robot_file(
    ...
    assert lines[6].startswith(
        "#"
    ), "The 7th line of this file doesn't start with #- have the headers changed? If so, update these lines of code!"

    # Updating the header of the robot file with the updated parameters (if any)
    header[
        2
    ] = f"Radial_Offset_Adjustment, {offset} #Radial offset is in millimetre(mm) with +ve values actioning radial outward movement and -ve values actioning radial inward movement of the magnets.\n"
    header[
        5
    ] = f"Radial_Scale_factor, {round(offset/plate_radius,5) + 1} #Radial scale factor is thermal coefficient of invar times temperature difference applied radially relative to the plate centre.\n"

    if T_observed is not None and T_configured is not None:
        header[
            3
        ] = f"RobotTemp, {T_configured:.5f} #temperature (degrees C) the distortion code assumed the field would be observed at\n"
        header[
            4
        ] = f"ObsTemp, {T_observed:.5f} #actual temperature (degrees C) it is going to be observed at\n"
...
def apply_corrections(
    ...
    apply_rotation_correction=True,
    verbose=True,
    permagnet_theta_correction=True,
    metrology_sign="negative",
    rotation_axis_misalignment_sign="positive",
    apply_barrel_rotation_to_all_magnets=True,
    barrel_rotation_sign="positive",
    ...
    * A correction on North/South position of all magnets based on the roll of the robot arm as it places each object. The extension of the robot arm to the far corner of the plate induces a small torque which affects its ability to place a magnet accurately. This torque has been measured and fitted with quadratic function which depends on the y value (in robot coordinates) of the magnet and is applied to its x value.
    ...
    if T_observed is not None and T_configured is not None:
        if offset != 0.0:
            raise ValueError(
                "If T_configured and T_observed are given then offset can't be given as well!"
            )
    ...
    if (offset != 0) or (apply_telecentricity_correction is True):
        df = corrections.apply_offsets_to_magnets(
            df,
            offset,
            robot_centre,
            apply_telecentricity_correction=apply_telecentricity_correction,
    ...
    if apply_metrology_calibration:
        ...
        (
            metr_calibrated_coords,
            calibd_theta_d,
        ) = corrections.perform_metrology_calibration(
            orig_coords,
            theta_d,
            robot_centre=robot_centre,
            robot_shifts_file=robot_shifts_file,
            verbose=verbose,
            permagnet_theta_corr=permagnet_theta_correction,
            sign=metrology_sign,
        )
    ...
    if apply_roll_correction:
        ...
        for index, row in df.iterrows():
            roll_correction_offset = corrections.roll_correction(
                row["Center_x"], row["Center_y"], row["Magnet"]
            )
    ...
    if apply_rotation_correction:
        if verbose:
            print(
                f"\tApplying the offset-rotation-axis correction. Sign is {rotation_axis_misalignment_sign.upper()}"
            )
        for index, row in df.iterrows():
            new_x, new_y = corrections.pick_up_arm_rotation_correction(
                row["Center_x"],
                row["Center_y"],
                rot_platePlacing=row["rot_platePlacing"],
                sign=rotation_axis_misalignment_sign,
            )
    ...
    if apply_barrel_rotation_to_all_magnets:
        ...
        if barrel_rotation_sign == "positive":
            df["rot_platePlacing"] = (
                df["rot_platePlacing"] + overall_barrel_rotation_value
            )
...
def update_parking_positions(robotFile_fromLabView, updated_parkingPositions_filename):
    ...
    robotFile_fromLabView_copy = (
        robotFile_fromLabView.replace(".txt", "") + "_after_loading.txt"
    )
    ...
    df_robotFile = pd.read_csv(
        robotFile_fromLabView_copy, delimiter="\t", names=colnames
    )

    # Read the "ParkingPosn" file
    df_parkPosn = file_functions.read_parking_positions_file(
        updated_parkingPositions_filename
    )

    # The start/end indicies definitions listed on the LabView robot control main window
    for magnetRange, name in zip(
        [[0, 499], [500, 599], [600, 699], [700, 799], [800, 899]],
        [
            "RectangularMagnet",
            "BlueCircularMagnet",
            "GreenCircularMagnet",
            "YellowCircularMagnet",
            "MagnetaCircularMagnet",
        ],
    ):
        robot_info = df_robotFile.loc[
            (df_robotFile["PosStatus"] == 0)
            & (df_robotFile["MagnetNo"] >= magnetRange[0])  # pos=0 is the park position
            & (df_robotFile["MagnetNo"] <= magnetRange[1])
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "robot_filename",
        type=str,
        help="The full path of the robot file to apply corrections to",
    )
    ...
    parser.add_argument(
        "robot_shifts_file",
        type=str,
        help="Filename of the robot metrology measurements",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Robot tile with LabView date-stamp",
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=float,
        help="Radial offset to apply in mm. +ve is outward, -ve is inwards",
    )
    parser.add_argument(
        "--T_observed",
        default=None,
        type=float,
        help="Temperature at which the plate will be observed",
    )
    parser.add_argument(
        "--T_configured",
        default=None,
        type=float,
        help="Temperature at which the plate is configured",
    )
    parser.add_argument(
        "--silent", action="store_true", help="Turn off output from the code"
    )
    ...
    parking_positions_df, correct_parkingpos_file = correct_parking_positions_file(
        parking_positions_filename,
        robot_shifts_file=robot_shifts_file,
        verbose=verbose,
        apply_metrology_calibration=True,
